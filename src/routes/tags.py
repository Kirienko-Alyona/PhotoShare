...
@router.post("/", response_model=PhotoResponse, name="Create tag to photo", status_code=status.HTTP_201_CREATED)
async def create_tag(body: TagModel, db: Session = Depends(get_db),
                     current_user: User = Depends(auth_service.get_current_user)):
    photo = await repository_tags.add_tag(body, db, current_user)
    if not photo:
