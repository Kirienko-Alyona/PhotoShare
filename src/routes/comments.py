...
from src.schemas.comments import CommentResponse, CommentModel
...
async def create_comment(body: CommentModel, db: Session = Depends(get_db),
    ...
    comment = await repository_comments.add_comment(db, current_user, body)
    return comment
...
async def update_comment(body: CommentModel, comment_id: int = Path(ge=1), db: Session = Depends(get_db),
    ...
    comment = await repository_comments.update_comment(db, comment_id, body)
    return comment
...
async def remove_comment(comment_id: int = Path(ge=1), db: Session = Depends(get_db),
                         _: User = Depends(auth_service.get_current_user)):
    comment = await repository_comments.remove_comment(db, comment_id)
    if comment is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=messages.NOT_FOUND)
    return comment
...
async def get_comments_by_photo(photo_id: int, db: Session = Depends(get_db),
                                  _: User = Depends(auth_service.get_current_user)):
    comments = await repository_comments.get_comments_by_photo(db, photo_id)
    if comments is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=messages.NOT_FOUND)
    return comments
